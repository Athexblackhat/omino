...
if [[ "$MODE" = "stealth" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "omino -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/$TARGET-$MODE.txt    
    ...
    omino $args | tee $LOOT_DIR/output/omino-$TARGET-$MODE-`date +"%Y%m%d%H%M"`.txt 2>&1
    exit
  fi
  ...
  echo "omino -t $TARGET -m $MODE --noreport $args" >> $LOOT_DIR/scans/running_${TARGET}_stealth.txt
