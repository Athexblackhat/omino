...
if [[ "$MODE" = "fullportonly" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "omino -t $TARGET -m $MODE --noreport " >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt
  ...
  if [[ -f "/usr/share/omino/pro/.portscanner.conf" ]]; then
    source /usr/share/omino/pro/.portscanner.conf
  fi
  if [[ -z "$PORT" ]]; then
