...
if [[ "$MODE" = "webporthttps" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "[ominosecurity.com] •?((¯°·._.• Started omino scan: https://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[ominosecurity.com] •?((¯°·._.• Started omino scan: https://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
  ...
  echo -e "$RESET"
  echo -e "$OKORANGE + -- --=[https://ominosecurity.com"
  ...
  then
    ...
    if [[ "$BLACKWIDOW" == "1" ]]; then
      ...
      if [[ $(wc -c $LOOT_DIR/web/spider-new-$TARGET.txt | awk '{print $1}') > 3 ]]; then
        echo "[ominosecurity.com] •?((¯°·._.• Spider URL change detected on $TARGET (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    ...
    if [[ "$DIRSEARCH" == "1" ]]; then
      ...
      if [[ $(wc -c $LOOT_DIR/web/dirsearch-new-$TARGET.txt| awk '{print $1}') > 3 ]]; then
        echo "[ominosecurity.com] •?((¯°·._.• Disovered URL change detected on $TARGET (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
  ...
  if [[ "$BURP_SCAN" == "1" ]]; then
    ...
    echo "[ominosecurity.com] •?((¯°·._.• Finished omino scan: https://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[ominosecurity.com] •?((¯°·._.• Finished omino scan: https://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
    if [[ "$LOOT" = "1" ]]; then
      loot
    fi
