...
display_banner() {
    ...
    echo ""
    echo -e "${DIM}  Powered by ATHEX | Multi-Distro Support | Auto-Dependency Resolution${RESET}"
    echo ""
...
confirm_installation() {
    ...
    echo ""
    printf "${OKRED}${BOLD}"
    pulse_effect "    This will install OMINO under $INSTALL_DIR. Continue?" "$OKRED" 2
    printf "${RESET}"
    echo ""
...
main() {
    ...
    section_header "Phase 4: Security Arsenal" "" "$OKRED"
    ...
    section_header "Phase 11: Configuration" "" "$OKBLUE"
    if [[ "$OS" != "macos" ]]; then
